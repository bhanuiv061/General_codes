...
#                              "Float 0.0-1.0 = fraction of height (e.g. 0.5 = center). "
...
def draw_count_panel(frame, count_in, all_classes, h, w):
    """Semi-transparent panel on top-right showing per-class IN count only. Only shows classes that have been detected
    at least once.
    """
    panel_x = w - 280
    panel_y = 10
    ...
    # Semi-transparent background
    overlay = frame.copy()
    cv2.rectangle(overlay, (panel_x - 10, panel_y), (w - 5, panel_y + panel_h), (20, 20, 20), -1)
    cv2.addWeighted(overlay, 0.65, frame, 0.35, 0, frame)

    # Gold border
    cv2.rectangle(frame, (panel_x - 10, panel_y), (w - 5, panel_y + panel_h), (0, 215, 255), 2)

    # Header
    cv2.putText(
        frame, "CLASS            COUNT", (panel_x, panel_y + 28), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 215, 255), 2
    )

    # Divider
    cv2.line(frame, (panel_x - 10, panel_y + 35), (w - 5, panel_y + 35), (0, 215, 255), 1)
    ...
    for i, cls in enumerate(vis_cls):
        ...
        cv2.putText(frame, text, (panel_x, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
    ...
    try:
        ...
        for data in dataset:
            ...
            if pred[0] is not None and len(pred[0]):
                ...
                masks_tensor = process_mask(proto[0], pred[0][:, 6:], pred[0][:, :4], (h_frame, w_frame), upsample=True)

                for i, (*xyxy, conf, cls) in enumerate(pred[0][:, :6]):
                    x1, y1, x2, y2 = map(int, xyxy)
                    detections.append(
                        [x1, y1, x2, y2, conf.item(), int(cls), masks_tensor[i].cpu().numpy().astype(bool)]
                    )
            ...
            # Buffer lines (gold)
            cv2.line(frame, (0, LINE_Y - BUFFER_PX), (w_frame, LINE_Y - BUFFER_PX), (255, 215, 0), 1)
            cv2.line(frame, (0, LINE_Y + BUFFER_PX), (w_frame, LINE_Y + BUFFER_PX), (255, 215, 0), 1)
            # Main line (cyan)
            cv2.line(frame, (0, LINE_Y), (w_frame, LINE_Y), (0, 255, 255), 2)

            # Direction label — center of line
            mid_x = w_frame // 2
            cv2.putText(
                frame,
                "▼  COUNTING LINE  (top → bottom)",
            ...
            if raw_writer is None:
                fps = vid_cap.get(cv2.CAP_PROP_FPS) if vid_cap else 25
                fps = fps if fps > 0 else 25
                raw_writer = cv2.VideoWriter(str(raw_video), cv2.VideoWriter_fourcc(*"mp4v"), fps, (w_frame, h_frame))
                ann_writer = cv2.VideoWriter(str(ann_video), cv2.VideoWriter_fourcc(*"mp4v"), fps, (w_frame, h_frame))
