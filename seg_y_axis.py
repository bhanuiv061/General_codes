...
# def draw_panel(frame, count_in, count_out):
#     h, w = frame.shape[:2]
#     x = w - 300
#     y = 10

#     overlay = frame.copy()
#     cv2.rectangle(overlay, (x-10,y), (w-5,y+220), (20,20,20), -1)
#     cv2.addWeighted(overlay, 0.6, frame, 0.4, 0, frame)

#     cv2.putText(frame, "CLASS   IN  OUT", (x,y+25),
#                 cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,215,255),2)

#     for i, cls in enumerate(count_in.keys()):
#         text = f"{cls[:8]:<8}{count_in[cls]:>4}{count_out[cls]:>4}"
#         cv2.putText(frame, text, (x,y+60+i*30),
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255),2)

# # ================= CROSS LOGIC =================
# def crossed_line(prev_cx, prev_cy, cx, cy, line_pos, direction):
#     if direction == "vertical":
#         return (prev_cy < line_pos and cy >= line_pos), \
#                (prev_cy > line_pos and cy <= line_pos)
#     else:
#         return (prev_cx < line_pos and cx >= line_pos), \
#                (prev_cx > line_pos and cx <= line_pos)
...
#         count_out = {cls:0 for cls in names.values()}
...
#                     crossed_in, crossed_out = crossed_line(
#                         prev_cx, prev_cy, cx, cy, LINE_POS, direction
#                     )
...
#             draw_panel(frame,count_in,count_out)
...
#         print("\nFINAL COUNT:")
#         for k in count_in:
#             print(f"{k} -> IN:{count_in[k]} OUT:{count_out[k]}")
...
def draw_panel(frame, count_in):
    ...
    cv2.putText(frame, "CLASS   IN", (x + 5, y + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 215, 255), 2)

    for i, cls in enumerate(count_in.keys()):
        text = f"{cls[:10]:<10}{count_in[cls]:>4}"
        cv2.putText(frame, text, (x + 5, y + 60 + i * 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    ...
    try:
        ...
        for data in dataset:
            ...
            for trk in tracks.astype(int):
                ...
                cv2.putText(frame, f"{cls_name}#{tid}", (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
...
#     cv2.putText(frame, "CLASS   IN", (x + 10, y + 25),
#                 cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 215, 255), 2)

#     for i, cls in enumerate(count_in.keys()):
#         text = f"{cls[:10]:<10}{count_in[cls]:>4}"
#         cv2.putText(frame, text, (x + 10, y + 60 + i * 30),
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
...
#                         track_class[tid] = names[cls_id]
#                     else:
#                         print(f"⚠ Invalid class id detected: {cls_id}")
#                         continue
